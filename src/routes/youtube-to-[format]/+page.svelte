<script lang="ts">
  ...
  const fetchDownloadAudio = (async () => {
    $loading = true;
    try {
      const audio_url = $global_selectedAudio.url;
      const filename = $global_selectedAudio.title;
      const to = params;
      const response = await fetch(
        `${env.PUBLIC_API_AUDIO_DOWNLOAD_URL}?url=${youTubeURL}&filename=${filename}&to=${to}`,
        {
          method: "POST",
          headers: {
  ...
  const fetchDownloadVideo = (async () => {
    $loading = true;
    try {
      const video_url = $global_selectedVideo.url;
      const filename = $global_selectedVideo.title;
      const to = params;
      const response = await fetch(
        `${env.PUBLIC_API_VIDEO_DOWNLOAD_URL}?url=${youTubeURL}&filename=${filename}&to=${to}`,
        {
          method: "POST",
          headers: {
            "Content-type": "application/json",
      ...
      if (!response.ok) {
        throw new Error("Get Video data failed.");
      }
...
{#if $fileFormat === "video"}
  ...
  <div class="sm:px-6 md:px-12 lg:px-12 py-2 mt-6">
    <div class="sm:col md:flex md:gap-6 lg:gap-10">
      <a
        href={$global_selectedVideo.url}
        class="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row w-full hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700"
      >
        <img
          class="object-cover w-full rounded-t-lg h-full md:h-full md:w-48 md:rounded-none md:rounded-l-lg"
          src={$global_selectedVideo.thumbUrl}
          alt=""
        />
        <div class="flex flex-col justify-between p-4 leading-normal">
          <h5
            class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white"
          >
            {$global_selectedVideo.title}
          </h5>
          <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">
            {$global_selectedVideo.url}
          </p>
        </div>
      </a>
    </div>

    {#await fetchDownloadVideo then}
      <div>
        <a
          target="_blank"
          rel="noopener"
          href={resultFileURL}
          download={$global_selectedVideo.title}
        >
          <p class="text-sm sm:text-sm md:text-base lg:text-lg">Download</p>
        </a>
      </div>
